...
import { CalendarEvent, MyEventsTabProps } from '@/types'
...
export function MyEventsTab({ userEvents, isLoadingEvents }: MyEventsTabProps) {
    ...
    if (isLoadingEvents) {
        return (
            <div>
                <h2 className="text-lg font-semibold mb-4">{t('myEvents')}</h2>
        ...
        return (
            <div>
                ...
                <div className="text-center py-8">
                    ...
                    <p className="text-muted-foreground">{t('noEventsJoined')}</p>
                    <p className="text-sm text-muted-foreground">{t('createOrJoinEvents')}</p>
                </div>
            </div>
        )
    }
        ...
        <div key={event.id} className="border rounded-lg p-3">
            <div className="flex items-center justify-between">
                ...
                <div className="text-sm text-muted-foreground flex items-center space-x-1">
                    ...
                    <span>
                        {event.participant_count} {event.participant_count === 1 ? t('participant') : t('participants')}
                    </span>
                </div>
            </div>
            ...
            <div className="mt-3 space-y-1">
                <p className="text-xs text-muted-foreground font-medium">{t('participants')}:</p>
    ...
    return (
        <div>
            ...
            <div className="space-y-6">
                {/* Future Events */}
                <div>
                    <h3 className="text-md font-medium mb-3 text-primary">{t('upcomingEvents')}</h3>
                ...
                {pastEvents.length > 0 && (
                    <div>
                        ...
                        >
                            <h3 className="text-md font-medium text-muted-foreground">{t('pastEvents')} ({pastEvents.length})</h3>
                            {showPastEvents ? (
